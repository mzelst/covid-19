...
setwd("C:/Users/s379011/surfdrive/projects/CoronaWatchNL")
...
daily_diff <- daily_diff %>%
  dplyr::filter(Datum > as.Date("2020-06-07"))
...
data_prov$Date <- as.numeric(data_prov$Datum)

data_prov_june <- data_prov %>%
  dplyr::filter(Date > 18412)

data_prov_normalized <- data_prov_june %>%
  group_by(Provincienaam) %>%
  mutate(Aantal = Aantal - Aantal[Date == 18413])



# Positief-geteste Coronavirus besmettingen per provincie
data_prov_normalized %>%
...
data_prov_june <- data_prov %>%
  filter(Datum > as.Date("2020-30-05"))
