...
find_month <- function(var) {
  round(as.numeric((var - trunc(var))*12 + 1))
}
...
nl_dt <- fread("data-rivm/municipal-datasets-per-day/rivm_municipality_perday_2021-04-18.csv.gz")
nl_dt <- aggregate(Deceased ~ Date_of_publication + Province, data = nl_dt, sum)

nl_dt <- nl_dt %>%
  filter(Province == "Noord-Brabant")

nl_dt$month <- month(nl_dt$Date_of_publication)
nl_dt$year <- year(nl_dt$Date_of_publication)
nl_dt <- aggregate(Deceased ~ month + year, data = nl_dt, sum)

colnames(nl_dt) <- c("month","year","covid_deaths")

## read cbsodata on weekly deaths
cbs_dt_months <- data.table(cbs_get_data('37230ned',Perioden = has_substring("MM"), RegioS = has_substring("PV30"))
)[,
  timestamp := Sys.time()
]

cbs_dt_months <- cbs_dt_months %>%
  select(Perioden, Overledenen_3) %>%
  mutate(year = as.numeric(substr(Perioden,1,4))) %>%
  mutate(month = as.numeric(substr(Perioden,7,8)))
...
nl_dt <- merge(cbs_dt_months[month %in% seq(1, 12),
                      .(cbs_deaths = Overledenen_3, year, month)
],
nl_dt,
by = c('year', 'month'),
all = T
...
cbs_deaths_ts <- ts(nl_dt$cbs_deaths, start = c(2002, 1), frequency = 12)
...
begin_end_list <- list(c(2017, 12, 2018, 4),
                       c(2016, 11, 2017, 3),
                       c(2016, 1, 2016, 3),
                       c(2014, 12, 2015, 4),
                       c(2012, 12, 2013, 4)
...
data.table(t = 2020 + (cbs_oversterfte[,1]-1)/12,
           model = 'CBS/AMC',
           covid_deaths = NA,
           variable = 'beta',
...
totals <- beta_long[t == 2020 & month %in% seq(3, 12),
                    .(month, cumsum(oversterfte)),
                    by=c('variable', 'model')
][,
...
fig2.1_dt <- data.table(year = round(as.numeric(trunc(time(covid_filt$y)))),
                        month = find_month(as.numeric(time(covid_filt$y))),
...
## create plot
ggplot(fig2.1_dt, aes(factor(month), cbs_deaths, group = 1)) +
...
fig4.1.1_dt <- beta_long[model == 'Dynamisch' & t >= 2020 & month >= 3,
                         .(mid = mean(value),
...
## write to excel
#write.xlsx(fig4.1.1_dt, 'output/figures.xlsx', sheetName = 'fig4.1.1', append = T)

## create plot
ggplot(fig4.1.1_dt, aes(factor(month), mid, group = 1)) +
  geom_line() +
  geom_errorbar(aes(ymax = upr, ymin = lwr), col = 'red', alpha = 0.4) + 
  xlab('Month') +
  ylab('Beta') + 
  theme_bw()
#ggsave('figs/fig4.1.1.png')

## Figure 4.1.2

## data
fig4.1.2_dt <- data.table(year = round(as.numeric(trunc(time(covid_filt$y)))),
                          month = find_month(time(covid_filt$y)),
                          cbs_deaths = covid_filt$y,
...
## write to excel
#write.xlsx(fig4.1.2_dt, 'output/figures.xlsx', sheetName = 'fig4.1.2', append = T)

## plot
ggplot(fig4.1.2_dt,
       aes(as.factor(month), mid, group = year)) +
  ...
  ylab('') + 
  theme_bw()
#ggsave('figs/fig4.1.2.png')

## figure 4.2.1
fig4.2.1_dt <-beta_long[t >= 2020 & month >= 3,
...
## plot
ggplot(fig4.2.1_dt, aes(factor(month), mid, group = 1)) +
