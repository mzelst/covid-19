...
all.deaths <- fread("https://ec.europa.eu/eurostat/estat-navtree-portlet-prod/BulkDownloadListing?file=data/demo_r_mwk_ts.tsv.gz")
colnames(all.deaths)[1] <- "identifier"

all.deaths <- all.deaths %>%
  dplyr::filter(identifier == "T,NR,AT")

all.deaths <- as.data.frame(t(all.deaths))
all.deaths$period <- row.names(all.deaths)
all.deaths <- all.deaths[2:nrow(all.deaths),]
all.deaths$week <- str_sub(all.deaths$period,6,7)
all.deaths$year <- str_sub(all.deaths$period,1,4)
all.deaths$cbs_deaths <- parse_number(all.deaths$V1)


temp <- tempfile()
download.file("https://covid19-dashboard.ages.at/data/data.zip", 
              temp)
con <- unz(temp, "CovidFaelle_Timeline.csv")
dat <- read.csv(con, header=T, sep=";")
unlink(temp)

dat$date <- str_sub(dat$ï..Time,1,10)
dat$covid_deaths <- dat$AnzahlTotTaeglich
dat <- dat %>%
  filter(Bundesland == "Ã–sterreich")

dat.austria <- aggregate(covid_deaths ~ date, data=dat,FUN=sum)

dat.austria$date <- str_replace_all(dat.austria$date,"[.]","-")
dat.austria$date <- dmy(dat.austria$date)
dat.austria$week <- isoweek(dat.austria$date)
dat.austria$year <- isoyear(dat.austria$date)
dat.austria <- aggregate(covid_deaths ~ week + year, data = dat.austria, FUN = sum)

nl_dt <- merge(all.deaths[,c("week","year","cbs_deaths")], dat.austria, by = c("week","year"), all.x=T)
nl_dt <- nl_dt %>%
  filter(year <= 2020) %>%
  filter(week <= 52) %>%
  mutate(week = as.numeric(week)) %>%
  mutate(year = as.numeric(year))
nl_dt$covid_deaths <- nafill(nl_dt$covid_deaths,fill=0)
nl_dt <- nl_dt[with(nl_dt,order(year,week)),]
write.csv(nl_dt, file = "workflow/excess_mortality/international/austria/all_data.csv",row.names = F)
nl_dt <- read.csv("workflow/excess_mortality/international/austria/all_data.csv")
nl_dt$covid_deaths <- as.numeric(nl_dt$covid_deaths)
nl_dt$week <- as.numeric(nl_dt$week)
...
write.csv(totals, file = paste0("workflow/excess_mortality/international/austria/dlm_2020.csv"))
...
write.csv(fig2.1_dt,file = "workflow/excess_mortality/international/austria/fig2.1_dt.csv")
...
ggplot(fig2.1_dt, aes(factor(week), cbs_deaths, group = 1)) +
  ...
  ylab('') + 
  theme_bw()
ggsave('workflow/excess_mortality/international/austria/fig2.1.png')
...
write.csv(fig2.2_dt,file = "workflow/excess_mortality/international/austria/fig2.2_dt.csv")
...
ggplot(fig2.2_dt, aes(factor(week), Gemiddeld)) +
  ...
  theme_bw()
ggsave('workflow/excess_mortality/international/austria/fig2.2.png')
...
write.csv(fig4.1.1_dt,file = "workflow/excess_mortality/international/austria/fig4.1.1_dt.csv")
...
ggplot(fig4.1.1_dt, aes(factor(week), mid, group = 1)) +
  ...
  ylab('Beta') + 
  theme_bw()
ggsave('workflow/excess_mortality/international/austria/fig4.1.1.png')
...
write.csv(fig4.1.2_dt,file = "workflow/excess_mortality/international/austria/fig4.1.2_dt.csv")
...
ggplot(fig4.1.2_dt,
  ...
  ylab('') + 
  theme_bw()
ggsave('workflow/excess_mortality/international/austria/fig4.1.2.png')
...
write.csv(fig4.2.1_dt,file = "workflow/excess_mortality/international/austria/fig4.2.1_dt.csv")
...
ggplot(fig4.2.1_dt, aes(factor(week), mid, group = 1)) +
  ...
  xlab('Week') + 
  theme_bw()
ggsave('workflow/excess_mortality/international/austria/fig4.2.1.png')
...
write.csv(fig4.2.2_dt,file = "workflow/excess_mortality/international/austria/fig4.2.2_dt.csv")
...
ggplot(fig4.2.2_dt, aes(variable, value)) +
  ...
  theme_bw()
ggsave('workflow/excess_mortality/international/austria/fig4.2.2.png')
