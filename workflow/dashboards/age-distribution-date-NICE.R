...
nice_by_day_clinical <- nice_by_day_clinical[,c("Leeftijd","Klinisch_aanwezig","Datum")]

perc <- nice_by_day_clinical %>% 
  group_by(Datum) %>% mutate(value = round((Klinisch_aanwezig/sum(Klinisch_aanwezig))*100,2))

nice_by_day_perc <- cbind(nice_by_day_clinical[,c("Leeftijd","Datum")],as.numeric(perc$value))
colnames(nice_by_day_perc) <- c("Leeftijd","Datum","Klinisch_aanwezig_Percentage")

Klinisch_aanwezig <- nice_by_day_clinical %>%
...
nice_by_day_IC <- nice_by_day_IC[,c("Leeftijd","IC_aanwezig","Datum")]

perc <- nice_by_day_IC %>% 
  group_by(Datum) %>% mutate(value = round((IC_aanwezig/sum(IC_aanwezig))*100,2))

nice_by_day_perc <- cbind(nice_by_day_IC[,c("Leeftijd","Datum")],as.numeric(perc$value))
colnames(nice_by_day_perc) <- c("Leeftijd","Datum","IC_Aanwezig_Percentage")

IC_Aanwezig <- nice_by_day_IC %>%
...
colnames(nice_by_day_clinical) <- c("Leeftijd","Aanwezig","Datum","Type")
colnames(nice_by_day_IC) <- c("Leeftijd","Aanwezig","Datum","Type")

nice_by_day <- rbind(nice_by_day_clinical, nice_by_day_IC)
write.csv(nice_by_day, file = "data-nice/age/leeftijdsverdeling_datum_Klinisch_IC_long.csv", row.names = F)
